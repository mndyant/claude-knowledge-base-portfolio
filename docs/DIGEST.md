...
個人運用時の`digest.yml`は19:30 UTC（PT昼）と01:30 UTC（PT夕方＝日本の朝10:30）に実行していた。
2回目が日本の朝に届くよう設計している。PTの夏冬時間差（±1h）とActionsの
cron遅延は日次ダイジェスト用途では許容範囲。
なお公開版の`digest.yml`は二重配信を防ぐため手動実行（`workflow_dispatch`）のみに変更している。
...
（`collect_docs_pages`参照）。RAG側のチャンキングも現在は追従済み（後述）。
...
- ~~split_llms_full.pyのページ境界検出が新フォーマットに未追従~~
  → 解消済み。`split_llms_full.py`の`_is_page_separator`はURLマーカー行
  （`PAGE_URL_RE`）もページ境界として扱い、両形式に対応した
  （`tests/test_split_llms_full.py`の7月版フォーマットのテストで検証）。
